...
export default function ComponentsGround(props: ComponentGroundProps) {
  ...
  const condition = !!participant.Firstname;
  const [firstText, setFirstText] = useState(
    condition ? participant.Firstname : "",
  );
  const [lastText, setLastText] = useState(
    condition ? participant.Lastname : "",
  );
  const [usernameText, setUsernameText] = useState(
    condition ? participant.Username : "",
  );
  const [DOBText, setDOBText] = useState<Date | undefined>(
    condition ? participant.DOB : undefined,
  );
  useEffect(() => {
    setDOB(DOBText);
  }, [DOBText, setDOB]);
  const [imageUrl, setimageUrl] = useState(
    condition && participant.Image
  ...
  useEffect(() => {
    autoSave();
  }, [firstText, lastText, usernameText, DOBText, gender, imageUrl]);
